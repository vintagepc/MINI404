...
static void test_rising_edge_trigger(void)
{
	...
	for (int i=0; i<16; i++)
	{
		...
		writel(STM32_RI_ADDRESS(base, RI_RTSR1), 1U << i);
		writel(STM32_RI_ADDRESS(base, RI_FTSR1), 0);
		...
		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_RPR1)), ==, 1U << i);
		// clear flag:
		writel(STM32_RI_ADDRESS(base, RI_RPR1), 1U << i);

		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_RPR1)), ==, 0);
		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_FPR1)), ==, 0);
...
static void test_falling_edge_trigger(void)
{
	...
	for (int i=0; i<16; i++)
	{
		g_assert_false(qtest_get_irq(global_qtest, i));
		// Defaults are all GPIOA pins - enable Rising edge trigger and verify.
		writel(STM32_RI_ADDRESS(base, RI_RTSR1), 0);
		writel(STM32_RI_ADDRESS(base, RI_FTSR1), 1U << i);
		qtest_irq_clear_pulses(global_qtest);
		qtest_set_irq_in(global_qtest, "/machine/soc/EXTI", NULL, i, EXTI_FALLING);
		for (int j=0; j<16; j++)
			g_assert_cmpint(qtest_get_irq_pulsed(global_qtest, j), ==, i==j);
		...
		// Check interrupt flag(s) are correct:
		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_FPR1)), ==, 1U << i);
		// clear flag:
		writel(STM32_RI_ADDRESS(base, RI_FPR1), 1U << i);
...
static void test_sw_trigger(void)
{
	...
	for (int i=0; i<16; i++)
	{
		qtest_irq_clear_pulses(global_qtest);
		writel(STM32_RI_ADDRESS(base, RI_RTSR1), 0);
		writel(STM32_RI_ADDRESS(base, RI_SWIER1), 1U << i);
		for (int j=0; j<16; j++)
		{
			g_assert_cmpint(qtest_get_irq_pulsed(global_qtest, j), ==, i==j);
		}
		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_RPR1)), ==, 1U << i);
		writel(STM32_RI_ADDRESS(base, RI_RPR1), 0xFFFF);
		g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_FPR1)), ==, 0);
	}
	// test multiple pins at once
	qtest_irq_clear_pulses(global_qtest);
	writel(STM32_RI_ADDRESS(base, RI_SWIER1), 0b111100001111);
	for (int j=0; j<16; j++)
	{
		g_assert_cmpint(qtest_get_irq_pulsed(global_qtest, j), ==, (bool)((1U <<j) &  0b111100001111));
	}
	g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_RPR1)), ==, 0b111100001111);
	g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_FPR1)), ==, 0);
	writel(STM32_RI_ADDRESS(base, RI_RPR1), 0xFFFF);
	qtest_irq_clear_pulses(global_qtest);
}
...
static void test_mapping(void)
{
	...
	writel(STM32_RI_ADDRESS(base, RI_RTSR1), 0xFFFF);
	writel(STM32_RI_ADDRESS(base, RI_FTSR1), 0xFFFF);
	g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_RTSR1)), ==, 0xFFFF);
	g_assert_cmphex(readl(STM32_RI_ADDRESS(base, RI_FTSR1)), ==, 0xFFFF);
	...
	for (int i=0; i<gpio_count; i++)
	{
		...
		// Clear.
		writel(STM32_RI_ADDRESS(base, RI_RPR1), 0xFFFF);
		writel(STM32_RI_ADDRESS(base, RI_FPR1), 0xFFFF);
