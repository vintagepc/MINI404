/*
 * QTest testcase for the STM32 Flash Interface module.
 *
...
#include "../../hw/arm/prusa/stm32g070/stm32g070_flashint_regdata.h"
...
{
	uint32_t base = stm32g070xx_cfg.perhipherals[STM32_P_FINT].base_addr;
	...
	for (int i=0; i<stm32g070xx_cfg.flash_variants[0].mem_size; i+=4)
	{
		writel(stm32g070xx_cfg.flash_base + i, 0xDEADBEEF);
		g_assert_cmphex(readl(stm32g070xx_cfg.flash_base + i), == , 0);
	}
}
...
static void test_unlock(void)
{
	...
	r.raw = readl(STM32_RI_ADDRESS(base, RI_CR));
	g_assert_true(r.LOCK);
	writel(stm32g070xx_cfg.flash_base, 0xDEADBEEF);
	g_assert_cmphex(readl(stm32g070xx_cfg.flash_base), == , 0);
	...
	writel(stm32g070xx_cfg.flash_base, 0xDEADBEEF);
	g_assert_cmphex(readl(stm32g070xx_cfg.flash_base), == , 0xDEADBEEF);
	writel(stm32g070xx_cfg.flash_base, 0xCAFE);
	g_assert_cmphex(readl(stm32g070xx_cfg.flash_base), == , 0xCAFE);
...
static void test_erase(void)
{
	...
	for (int i=0; i<stm32g070xx_cfg.flash_variants[0].mem_size; i+=4)
	{
		writel(stm32g070xx_cfg.flash_base + i, i);
	}
	...
	for (int i=0; i<stm32g070xx_cfg.flash_variants[0].mem_size; i+=4)
	{
		g_assert_cmphex(readl(stm32g070xx_cfg.flash_base + i), == , i);
	}
	r.STRT = 1;
	writel(STM32_RI_ADDRESS(base, RI_CR),r.raw);
	...
	for (int i=0; i<stm32g070xx_cfg.flash_variants[0].mem_size; i+=4)
	{
		...
		{
			g_assert_cmphex(readl(stm32g070xx_cfg.flash_base + i), == , 0xFFFFFFFF);
		}
	}
}
...
int main(int argc, char **argv)
{
    ...
    qtest_add_func("/stm32_flashint/test_lock", test_lock);
    qtest_add_func("/stm32_flashint/test_unlock", test_unlock);
    qtest_add_func("/stm32_flashint/test_erase", test_erase);
